...
face_cascade_name = 'haarcascade_frontalface_alt.xml'  # 얼굴 식별 
...
def detectAndDisplay(image):
    ...
    show_frame = image
    frame_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)  # BGR -> GRAY
    frame_gray = cv2.equalizeHist(frame_gray)  # 인식률을 높이기 위한 필터링 과정 
    ...
    for (x,y,w,h) in faces:
        ...
        for (i, point) in enumerate(show_parts):  # 눈에 노란점 
            x = point[0,0]
            y = point[0,1]
